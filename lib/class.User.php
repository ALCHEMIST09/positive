<?php
	...
	class User extends DatabaseObject
	{
		...
		protected static $db_fields = array('id', 'fname', 'lname', 'username', 'password');
		public    $id;
		protected $_fname;
		protected $_lname;
		protected $_username;
		...
		function __toString()
		{
			...
			$html .= "<strong>Password:</strong> ".$this->_password;
		 ...
		 * Overides the default delete() method inheritied from DatabaseObject Class
		 * @todo Delete the user's entries in the user2collection table that keeps
		 * track the different groups the user belongs to
		 * @return boolean
		 */
		public function delete()
		{
			...
			$mysqli = $db->getConnection();
			$mysqli->autocommit(false);
			$sql = "DELETE FROM user2collection WHERE user_id = ".$this->id;
			$mysqli->query($sql);
